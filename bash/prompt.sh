...
function prompt-git-info {
  ...
  if [[ $? -eq 0 ]]; then
    ...
    # Get current status
    while IFS=$"\n" read line; do
      [[ "$line" =~ ([ACDMT][\ MT]|[ACMT]D)\ * ]] && local added="${COLOR_GREEN}●${COLOR_RESET}"
      [[ "$line" =~ [\ ACMRT]D\ * ]]              && local deleted="${COLOR_RED}●${COLOR_RESET}"
      [[ "$line" =~ ?[MT]\ * ]]                   && local modified="${COLOR_RED}●${COLOR_RESET}"
      [[ "$line" =~ R\ * ]]                      && local added="${COLOR_GREEN}●${COLOR_RESET}"
      [[ "$line" =~ (AA|DD|U?|?U)\ * ]]           && local modified="${COLOR_RED}●${COLOR_RESET}"
